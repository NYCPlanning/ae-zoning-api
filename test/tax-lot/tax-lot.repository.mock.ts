...
export class TaxLotRepositoryMock {
  ...
  async findGeomBuffer(
    ...
    return generateMock(findGeomBufferRepoSchema, {
      stringMap: {
        buffer: () => JSON.stringify(bufferGeom),
      },
    });
  }
  ...
  async checkGeomIsValid(geom: Geom): Promise<CheckGeomIsValidRepo> {
    ...
    if (checkGeom.shape.type === "LineString") {
      ...
      return { valid: uniqueCoordinates.size > 1 };
    }
    ...
    if (checkGeom.shape.type === "Polygon") {
      ...
      return {
        valid:
          flatCoordinates[0] === flatCoordinates[flatCoordinates.length - 1],
      };
    }
  ...
  async findMaximumInscribedCircleCenter(
    ...
    return { center: JSON.stringify(geomMock) };
  }
