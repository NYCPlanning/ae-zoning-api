/**
 * Generated by Kubb (https://kubb.dev/).
 * Do not edit manually.
 */

import { communityDistrictSchema } from "./communityDistrictSchema.ts";
import { errorSchema } from "./errorSchema.ts";
import { z } from "zod";

export const findCommunityDistrictsByBoroughIdPathParamsSchema = z.object({
  boroughId: z
    .string()
    .regex(/^([0-9]{1})$/)
    .describe(
      "A single character numeric string containing the common number used to refer to the borough. Possible values are 1-5.",
    ),
});

/**
 * @description An object of community district schemas for the borough
 */
export const findCommunityDistrictsByBoroughId200Schema = z.object({
  communityDistricts: z.array(z.lazy(() => communityDistrictSchema)),
  order: z.coerce
    .string()
    .describe("Community district numbers are sorted in ascending order"),
});

/**
 * @description Invalid client request
 */
export const findCommunityDistrictsByBoroughId400Schema = errorSchema;

/**
 * @description Requested resource does not exist or is not available
 */
export const findCommunityDistrictsByBoroughId404Schema = errorSchema;

/**
 * @description Server side error
 */
export const findCommunityDistrictsByBoroughId500Schema = errorSchema;

export const findCommunityDistrictsByBoroughIdQueryResponseSchema =
  findCommunityDistrictsByBoroughId200Schema;
