/**
 * Generated by Kubb (https://kubb.dev/).
 * Do not edit manually.
 */

import { errorSchema } from "./errorSchema.ts";
import { zoningDistrictClassCategoryColorSchema } from "./zoningDistrictClassCategoryColorSchema.ts";
import { z } from "zod";

/**
 * @description An object containing all zoning district category colors.
 */
export const findZoningDistrictClassCategoryColors200Schema = z.object({
  zoningDistrictClassCategoryColors: z.array(
    z.lazy(() => zoningDistrictClassCategoryColorSchema),
  ),
});

/**
 * @description Invalid client request
 */
export const findZoningDistrictClassCategoryColors400Schema = errorSchema;

/**
 * @description Server side error
 */
export const findZoningDistrictClassCategoryColors500Schema = errorSchema;

export const findZoningDistrictClassCategoryColorsQueryResponseSchema =
  findZoningDistrictClassCategoryColors200Schema;
