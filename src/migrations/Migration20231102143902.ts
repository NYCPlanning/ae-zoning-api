...
export class Migration20231102143902 extends Migration {
  async up(): Promise<void> {
    this.addSql(
      'create table "zoning_district" ("id" uuid not null, "label" text not null, "wgs84" geography(MultiPolygon, 4326) not null, "li_ft" geometry(MultiPolygon, 2263) not null, constraint "zoning_district_pkey" primary key ("id"));',
    );

    this.addSql(
      'create table "zoning_district_class" ("id" text not null, "category" text check ("category" in (\'Residential\', \'Commercial\', \'Manufacturing\')) not null, "description" text not null, "color" char(9) not null, constraint "zoning_district_class_pkey" primary key ("id"));',
    );
